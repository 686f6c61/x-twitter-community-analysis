...
import type { SearchOptions, ScrapingJob, ProgressUpdate, DownloadedFile, ProviderType, ProviderConfig } from "../services/scraperApi"
...
export function QuickScrapingSection() {
  ...
  const [queryOperator, setQueryOperator] = useState<'AND' | 'OR'>('OR')
  ...
  const [stoppedData, setStoppedData] = useState<{ filename?: string; tweetsCount?: number } | null>(null)
  ...
  const startScraping = async () => {
    ...
    try {
      ...
      // Si es AND, envolver cada término en paréntesis: (#javascript) (@elonmusk) (AI)
      // Si es OR, simplemente separar con espacios: #javascript @elonmusk AI
      const combinedQuery = queryOperator === 'AND'
        ? queryTags.map(tag => `(${tag})`).join(' ')
        : queryTags.join(' ')
      ...
      const options: SearchOptions = {
        ...
        includeReplies, // DEPRECATED: Mantenido por compatibilidad
        includeConversations: includeReplies, // Nuevo método batch optimizado
  ...
  const stopScraping = async () => {
    ...
    try {
      const result = await scraperAPI.stopJob(currentJob.jobId)
      setIsRunning(false)
      setStatusMessage('Scraping detenido')
      if (wsRef.current) wsRef.current.close()

      // Show enrichment modal if data was saved
      if (result.filename && result.tweetsCount && result.tweetsCount > 0) {
        setStoppedData({ filename: result.filename, tweetsCount: result.tweetsCount })
        setShowEnrichModal(true)
      } else {
        setStatusMessage('Scraping detenido sin datos')
      }
    } catch (err: any) {
      setError(err.message)
    }
  ...
  const handleEnrich = async () => {
    if (!stoppedData?.filename) return

    try {
      setShowEnrichModal(false)
      setError(null)
      setStatusMessage('Enriqueciendo usuarios...')

      const result = await scraperAPI.enrichFile(stoppedData.filename)
      setStatusMessage(`✓ Enriquecimiento completado: ${result.filename}`)
      await loadDownloads()
      setStoppedData(null)
    } catch (err: any) {
      setError(`Error enriqueciendo: ${err.message}`)
      setStoppedData(null)
    }
  }

  const handleSaveWithoutEnrich = async () => {
    setShowEnrichModal(false)
    setStatusMessage(`✓ Guardado: ${stoppedData?.filename} (${stoppedData?.tweetsCount} tweets)`)
    await loadDownloads()
    setStoppedData(null)
  }
  ...
  const handleDiscard = async () => {
    ...
    try {
      setShowEnrichModal(false)
      await scraperAPI.deleteFile(stoppedData.filename)
      setStatusMessage('Datos descartados')
      await loadDownloads()
      setStoppedData(null)
    } catch (err: any) {
      setError(`Error descartando: ${err.message}`)
      setStoppedData(null)
    }
  }
    ...
    try {
      ...
      worker.onmessage = (e) => {
        ...
        if (message.type === 'complete') {
          ...
          // Calcular rango de fechas desde los tweets si no está explícito
          ...
          // Guardar JSON completo raw para exportación académica
          setRawData(data)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
      ...
      <ScraperConfig
        ...
        onQueryOperatorChange={setQueryOperator}
        mode={mode}
      ...
      <EnrichmentModal
        ...
        tweetsCount={stoppedData?.tweetsCount || 0}
        onEnrich={handleEnrich}
        onSaveWithoutEnrich={handleSaveWithoutEnrich}
